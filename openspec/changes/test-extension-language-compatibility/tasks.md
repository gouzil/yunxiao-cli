...
- [x] 1.3 增加显式开关控制的表驱动兼容性测试；启用时检查 Bash、Python、Node.js 和 Go，使用真实 `LocalManager` 与 `RealExecRunner` 安装和分发，任一运行时缺失或统一契约断言失败即报错
...
- [x] 3.2 更新 `.github/workflows/ci.yml` 的 Linux job，显式配置 Python 和 Node.js 主版本，运行 `bash -n`、Ruff check、`node --check`、Go vet/build，并启用四语言兼容性测试；其他平台保持现有测试路径
- [x] 3.3 运行 `prek run --all-files`、目标多语言测试、`go test ./...`、`go vet ./...` 和 `openspec validate test-extension-language-compatibility --strict`，确认 fixture 可格式化检查、临时产物不写入工作区且全部检查通过
