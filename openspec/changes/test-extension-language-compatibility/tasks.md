...
- [ ] 3.3 运行 `prek run --all-files`、默认 `go test ./...`、带 tag 的目标多语言测试、Windows acceptance 交叉编译、`go vet ./...` 和 `openspec validate test-extension-language-compatibility --strict`，确认默认单测不报告兼容性测试跳过、fixture 可格式化检查、临时产物不写入工作区且全部检查通过
